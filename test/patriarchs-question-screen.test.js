...
const abraham = path.join(ROOT, "assets", "characters", "abram", "question.webp");

assert(index.includes('id="question-abraham"'), "question screen has an Abraham layer");
assert(index.includes('data-src="assets/characters/abram/question.webp"'), "Abraham uses the transparent question artwork");
...
assert(/moses\/question\.webp/.test(play), "Exodus swaps in the Moses question artwork");
assert(/gideon\/question\.webp/.test(play), "Judges swaps in the Gideon question artwork");
assert(/solomon\/question\.webp/.test(play), "Jerusalem, Shiloh, Tyre, and Samaria use Solomon");
assert(/joshua\/question\.webp/.test(play), "Jericho and Gilgal use Joshua");
assert(/elijah\/question\.webp/.test(play), "Carmel and Megiddo use Elijah");
assert(/elisha\/question\.webp/.test(play), "Lachish and Damascus use the Elisha plate");
assert(/jonah\/question\.webp/.test(play), "Nineveh uses Jonah");
assert(/daniel\/question\.webp/.test(play), "Babylon and Susa use Daniel");
assert(/samson\/question\.webp/.test(play), "Gibeah and Mizpah use Samson");
assert(/jesus\/question\.webp/.test(play), "Bethlehem, Nazareth, Capernaum, Golgotha, and Emmaus use Jesus");
assert(/baptist\/question\.webp/.test(play), "The Jordan uses John the Baptist");
assert(/paul\/question\.webp/.test(play), "Damascus Road through Rome use Paul");
assert(/john-revelator\/question\.webp/.test(play), "Patmos uses John the Revelator");
const moses = path.join(ROOT, "assets", "characters", "moses", "question.webp");
assert(fs.existsSync(moses) && fs.statSync(moses).size > 1000, "Moses question artwork is present");
const gideon = path.join(ROOT, "assets", "characters", "gideon", "question.webp");
assert(fs.existsSync(gideon) && fs.statSync(gideon).size > 1000, "Gideon question artwork is present");
const solomon = path.join(ROOT, "assets", "characters", "solomon", "question.webp");
assert(fs.existsSync(solomon) && fs.statSync(solomon).size > 1000, "Solomon question artwork is present");
const joshua = path.join(ROOT, "assets", "characters", "joshua", "question.webp");
assert(fs.existsSync(joshua) && fs.statSync(joshua).size > 1000, "Joshua question artwork is present");
const elijah = path.join(ROOT, "assets", "characters", "elijah", "question.webp");
assert(fs.existsSync(elijah) && fs.statSync(elijah).size > 1000, "Elijah question artwork is present");
const elisha = path.join(ROOT, "assets", "characters", "elisha", "question.webp");
assert(fs.existsSync(elisha) && fs.statSync(elisha).size > 1000, "Elisha question artwork is present");
const johnRevelator = path.join(ROOT, "assets", "characters", "john-revelator", "question.webp");
