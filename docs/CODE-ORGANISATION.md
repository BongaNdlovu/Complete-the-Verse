...
- **45 registered test suites + `test.js`** sit next to `index.html`. They belong in `test/`.
...
- `node test.js` is one command, 45 suites, names that match jobs (`recall logic`, `pilgrimage`, `cloud merge`).

### If they just clone the repo

**No.**

1. The first screen is 45 registered test suites and 12 autopsy reports.
...
- `node test.js` runs 45 named suites. A failure prints `FAIL <name>` plus stdout/stderr. You know *which job* broke.
...
| Edge score submit fails | Fails closed; local record remains and the board error is surfaced | Sync error chip and result toast |
